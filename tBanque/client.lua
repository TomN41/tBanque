...
RMenu.Add(':TomN | C-host.fr:', 'main', RageUI.CreateMenu("Banque", "Pour accéder à votre compte"))
RMenu.Add(':TomN | C-host.fr:', 'bank', RageUI.CreateSubMenu(RMenu:Get(':TomN | C-host.fr:', 'main'), "Compte", "Voici votre compte en banque."))
...
Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get(':TomN | C-host.fr:', 'main'), true, true, true, function()
            ...
            end, RMenu:Get(':TomN | C-host.fr:', 'bank'), verifpoche()) 

        end, function()
        end)

        RageUI.IsVisible(RMenu:Get(':TomN | C-host.fr:', 'bank'), true, true, true, function()

            RageUI.Button("~g~Déposer", nil, {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
            if (Selected) then   
                saisiedepot()
                verifsolde()
                verifpoche()
            ...
            RageUI.Button("~r~Retirer", nil, {RightLabel = "→→→"},true, function(Hovered, Active, Selected)
            if (Selected) then   
                saisieretrait()
                verifsolde()
                verifpoche()
            ...
            RageUI.Button("~o~Banque", nil, {RightLabel = argentsolde.."~g~ $"},true, function(Hovered, Active, Selected)
            if (Selected) then   
                verifsolde()
                verifpoche()
            end
            end)    
            RageUI.Button("~y~Portefeuille", nil, {RightLabel = argentpoche.."~g~ $"},true, function(Hovered, Active, Selected)
            if (Selected) then   
                verifpoche()
            end
            end)           
...
function verifsolde()
    TriggerServerEvent(":TomN | C-host.fr::solde", action)
end

function verifpoche()
    TriggerServerEvent(":TomN | C-host.fr::poche", action)
end

RegisterNetEvent(":TomN | C-host.fr::solderefresh")
AddEventHandler(":TomN | C-host.fr::solderefresh", function(money, cash)
    argentsolde = tonumber(money)
end)

RegisterNetEvent(":TomN | C-host.fr::pocherefresh")
AddEventHandler(":TomN | C-host.fr::pocherefresh", function(money, cash)
...
function saisieretrait()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent(':TomN | C-host.fr::retirer', amount)
				else
...
function saisiedepot()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent(':TomN | C-host.fr::deposer', amount)
				else
					ESX.ShowNotification("Vous n'avez pas saisi un montant")
				end
			end
